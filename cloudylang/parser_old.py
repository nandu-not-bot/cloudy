...
from .utils import TT, ParseResult, Position
...
class Parser:
    ...
    def statements(self):
        ...
        if self.current_tok.type == TT.SPACE:
            self.indent_level += (local_indent := self.current_tok.value)
            res.register_advancement()
            self.advance()
        ...
        while True:
            ...
            while self.current_tok.type == TT.NEWLINE:
                ...
                if not self.indent_level:
                    newline_count += 1
                elif self.current_tok.type == TT.SPACE:
                    if self.current_tok.value != self.indent_level:
                        return res.faliure(
                            InvalidSyntaxError(
                                self.current_tok.pos_start, self.current_tok.pos_end, "Uneven index"
                            )
                        )
                    newline_count += 1
            ...
            if self.indent_level and not self.current_tok.matches(TT.SPACE, self.indent_level):
                return res.faliure(
                    InvalidSyntaxError(
        ...
        if res.error:
            return res.faliure(
                InvalidSyntaxError(
                    pos_start,
                    self.current_tok.pos_end,
                    "Expected 'var', 'for', 'while', 'func', 'break', 'return', 'continue', int, float, identifier, '+', '-', '[' or '('",
                )
            )
    ...
    def list_expr(self):
        ...
        if self.current_tok.type != TT.LSQUARE:
            return res.faliure(
                InvalidSyntaxError(pos_start, self.current_tok.pos_end, "Expected '['")
            )
